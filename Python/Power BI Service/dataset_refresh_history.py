...
def get_refresh_history():
    ...
    for dataset_id, workspace_id in dataset_workspace:
        refresh_uri = rf"https://api.powerbi.com/v1.0/myorg/groups/{workspace_id}/datasets/{dataset_id}/refreshes"
...
def return_final_table():
    workspace_df = pd.DataFrame.from_records(
        get_workspaces(), 
        columns=["WorkspaceID", "WorkspaceName"]
    )
    dataset_df = pd.DataFrame.from_records(
        get_datasets(), 
        columns=["DatasetID", "DatasetName", "WorkspaceID"]
    )
    ...
    refresh_df["StartTime"] = pd.to_datetime(
        refresh_df["StartTime"]
        .apply(lambda x: datetime.fromisoformat(x))
    ...
    final_df = pd.merge(
        ...
        how="inner",
        left_on="WorkspaceID",
        right_on="WorkspaceID",
    ).merge(refresh_df, how="inner", on="WorkspaceID")
